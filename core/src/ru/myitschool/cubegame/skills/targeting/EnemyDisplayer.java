 ...
 */
public class EnemyDisplayer implements TargetDisplayer {

    @Override
    public boolean targetCreation(int x, int y, Array<TilePos> array, Skill skill) {
        if (DungeonMap.getCell(x, y) != null && DungeonMap.getCell(x, y).getEntity() != null && DungeonMap.getCell(x, y).getEntity().isEnemy()){
